...
#ifndef SKIACODEC_JAVA_INPUT_STREAM_ADAPTER_H
#define SKIACODEC_JAVA_INPUT_STREAM_ADAPTER_H
...
class JavaInputStreamAdaptor : public SkStream {
    JavaInputStreamAdaptor(JavaVM *jvm, jobject js, jbyteArray ar, jint capacity,
            ...
            : fJvm(jvm), fJavaInputStream(js), fJavaByteArray(ar), fCapacity(capacity),
              fBytesRead(0), fIsAtEnd(false), fSwallowExceptions(swallowExceptions) {}
...
SkStreamRewindable *CopyJavaInputStream(JNIEnv *env, jobject stream,
                                        jbyteArray storage) {
    std::unique_ptr<SkStream> adaptor(CreateJavaInputStreamAdaptor(env, stream, storage, true));
    if (NULL == adaptor.get()) {
        return NULL;
    }
...
int register_java_input_stream_adapter(JavaVM *jvm) {
    auto env = getEnv(jvm);
...
#endif //SKIACODEC_JAVA_INPUT_STREAM_ADAPTER_H
